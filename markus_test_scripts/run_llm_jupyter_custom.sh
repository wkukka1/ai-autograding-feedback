#!/bin/bash

# Define test parameters
TEST_NAME="LLM Feedback Test"
TEST_SCRIPT="test_hw5.py"
TEST_OUTPUT_FILE="test_output1.txt"

# Step 1: Run test_hw5.py to get test errors file
# Note: name as test_output1.txt, since CSV file is too large to load 
TEST_OUTPUT=$(/home/docker/.autotesting/scripts/defaultvenv/bin/pytest "$TEST_SCRIPT" 2>&1 | tee "$TEST_OUTPUT_FILE")
TEST_STATUS=${PIPESTATUS[0]} 

if [ $TEST_STATUS -eq 0 ]; then
  STATUS="pass"
  MARKS_EARNED=1
else
  STATUS="fail"
  MARKS_EARNED=0
fi
MARKS_TOTAL=1

#claude-3.7-sonnet
# Step 2: Run main.py to get LLM response 
MAIN_OUTPUT=$(/home/docker/.autotesting/scripts/defaultvenv/bin/python ai-autograding-feedback/main.py \
      --submission_type jupyter \
      --prompt code_lines \
      --scope code \
      --model openai \
      --output stdout 2>&1)

# Step 3: Clean up response for JSON output
OVERALL_COMMENT=$(echo "$MAIN_OUTPUT" | jq -R -s .)
JSON_TEST_OUTPUT=$(echo "$TEST_OUTPUT" | jq -R -s .)

echo "{\"name\": \"$TEST_NAME\", \"output\": "$JSON_TEST_OUTPUT", \"marks_earned\": $MARKS_EARNED, \"marks_total\": $MARKS_TOTAL, \"status\": \"$STATUS\", \"time\": null}  {\"overall_comment\": "$OVERALL_COMMENT"}"
